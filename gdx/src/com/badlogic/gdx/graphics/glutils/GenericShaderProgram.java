...
/** <p>
 * A generic shader program encapsulates a list of shaders linked to form a shader program useable with OpenGL ES > 2.0.
 * </p>
 * 
 * <p>
 * To make the GPU use a specific ShaderProgram the programs {@link com.badlogic.gdx.graphics.glutils.GenericShaderProgram#begin()}
 * method must be used which effectively binds the program.
 * </p>
 *
 * <p>
 * When a GenericShaderProgram is bound one can set uniforms, vertex attributes and attributes as needed via the respective methods.
 * </p>
 *
 * <p>
 * A GenericShaderProgram can be unbound with a call to {@link com.badlogic.gdx.graphics.glutils.GenericShaderProgram#end()}
 * </p>
 *
 * <p>
 * A GenericShaderProgram must be disposed via a call to {@link com.badlogic.gdx.graphics.glutils.GenericShaderProgram#dispose()} when it is no longer needed
 * </p>
 *
 * <p>
 * GenericShaderProgram are managed. In case the OpenGL context is lost all appShaders get invalidated and have to be reloaded. This
 ...
 *
 * @author mzechner */
public class GenericShaderProgram implements Disposable {
    ...
    protected static class Shader {
        ...
        Shader(int type) {
            this.type = type;
            this.handle = gl.glCreateShader(type);
        }
        ...
        String getLog() {
            return gl.glGetShaderInfoLog(handle);
        }
	...
	/** the list of currently available appShaders **/
	private final static ObjectMap<Application, Array<GenericShaderProgram>> appShaders = new ObjectMap<Application, Array<GenericShaderProgram>>();

    protected Map<Integer, Shader> shaders = new HashMap<Integer, Shader>();
    ...
    /** list of varyings to capture with transform feedback **/
    private String[] captures;

    /** the buffer mode used for transform feedback **/
    private int captureMode;

    /** Constructs a new GenericShaderProgram **/
	public GenericShaderProgram() {
	}

    /** Attaches a new shader to the program **/
    public Shader attachShader (int type, String source) {
        Shader shader = loadShader(type, source);
    ...
    /** Attaches a new shader to the program **/
    public Shader attachShader (int type, FileHandle file) {
        return attachShader(type, file.readString());
    }

	/** Loads and compiles the appShaders, creates a new program and links the appShaders. */
	public void link () {
		program = linkProgram();
		if (program == -1) {
			isCompiled = false;
	...
	private Shader loadShader (int type, String source) {
    ...
    /** captures the specified shader outputs to a single buffer **/
    public void captureInterleaved(String ...varyings) {
        capture(GL30.GL_INTERLEAVED_ATTRIBS, varyings);
    }

    /** captures the specified shader outputs to multiple buffers **/
    public void captureSeparate(String ...varyings) {
        capture(GL30.GL_SEPARATE_ATTRIBS, varyings);
    }

    private void capture(int bufferMode, String ...varyings) {
        captures = varyings;
        captureMode = bufferMode;
    }
	...
	private int linkProgram () {
        ...
        if (captures != null) {
            Gdx.gl30.glTransformFeedbackVaryings(program, captures, captureMode);
        }
...
// }
	...
	/** Sets the uniform with the given name. The {@link GenericShaderProgram} must be bound for this to work.
	 *
	...
	/** Sets the uniform matrix with the given name. The {@link GenericShaderProgram} must be bound for this to work.
	 *
	 * @param name the name of the uniform
	...
	/** Sets an array of uniform matrices with the given name. The {@link GenericShaderProgram} must be bound for this to work.
	 *
	 * @param name the name of the uniform
	...
	/** Sets the vertex attribute with the given name. The {@link GenericShaderProgram} must be bound for this to work.
	 *
	...
	public void dispose () {
		...
		if (appShaders.get(Gdx.app) != null) appShaders.get(Gdx.app).removeValue(this, true);
	}
	...
	private void addManagedShader (Application app, GenericShaderProgram shaderProgram) {
		Array<GenericShaderProgram> managedResources = appShaders.get(app);
		if (managedResources == null) managedResources = new Array<GenericShaderProgram>();
		managedResources.add(shaderProgram);
		appShaders.put(app, managedResources);
	}
	...
	public static void invalidateAllShaderPrograms (Application app) {
		...
		Array<GenericShaderProgram> shaderArray = appShaders.get(app);
	...
	public static String getManagedStatus () {
		...
		for (Application app : appShaders.keys()) {
			builder.append(appShaders.get(app).size);
			builder.append(" ");
		}
		builder.append("}");
